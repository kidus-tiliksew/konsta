...
const ToastClasses = (props, colors, classes) => {
  ...
  return {
    base: {
      common: (0, _cls.cls)((0, _positionClass.positionClass)('fixed', classes), 'z-50 transform transition-transform-opacity w-full sm:w-auto start-0 bottom-0 sm:max-w-lg', !opened && 'translate-y-full opacity-0 pointer-events-none'),
      ios: (0, _cls.cls)(`sm:rounded-lg duration-300`, colors.textIos, colors.bgIos, translucent && 'translucent'),
      material: (0, _cls.cls)(`sm:rounded-2xl duration-200`, colors.textMaterial, colors.bgMaterial),
